...
class My_Classifier_Model:
    def __init__(self):
        logger.info("Инициализация My_Classifier_Model")
    ...
    def preprocess(self, df, training=True):
        ...
        try:
            ...
            logger.info("Предобработка завершена <3")
            if training:
    ...
    def train(self, dataset_path):
        """Обучение модели -_-"""
        try:
            logger.info(f"Запуск обучения. Датасет: {dataset_path}")
    ...
    def predict(self, dataset_path):
        """Предсказание модели ww"""
        try:
            logger.info(f"Запуск предсказания. Данные: {dataset_path}")
